...
<%def name="container_content()" filter="trim">
        <form action="${dataset.get_admin_url(ctx, 'edit')}" method="post" role="form">
            <%self:hidden_fields/>
            <fieldset>
                <legend>${_(u'Edition of %s') % dataset.get_title(ctx)}</legend>
                <%self:error_alert/>
                <%self:form_fields/>
...
${_(u'Edit')} - ${dataset.get_title(ctx)} - ${parent.title_content()}
</%def>

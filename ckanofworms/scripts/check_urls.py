...
def check_dataset_urls(dataset, state = None):
    try:
        changed = False
        dataset_errors = dataset.get('errors') or {}
        changed = set_error(dataset_errors, 'url', validate_url(dataset.get('url'), state = state)) or changed

        resources_errors = dataset_errors.get('resources') or {}
        for resource_index, resource in enumerate(dataset.get('resources') or []):
            ...
            changed = set_error(resource_errors, 'url', validate_url(resource.get('url'), state = state)) or changed
            if resource_errors:
                resources_errors[resource_index_str] = resource_errors
            else:
                resources_errors.pop(resource_index_str, None)
        if resources_errors:
            dataset_errors['resources'] = resources_errors
        else:
            dataset_errors.pop('resources', None)
        if dataset_errors:
            dataset['errors'] = dataset_errors
        else:
            dataset.pop('errors', None)

        if changed:
            log.info(u'Updating dataset: {}'.format(dataset['name']))
            request_headers = headers.copy()
            request_headers['Content-Type'] = 'application/json'
            request = urllib2.Request(urlparse.urljoin(conf['ckan_of_worms.site_url'], 'api/1/datasets'),
            ...
            try:
                ...
                if response.code == 409:
                    # The dataset has been modified. Don't submit it because we will be notified of its new version.
                    log.info(u'Dataset "{}" has been modified. URLs checks are ignored.'.format(dataset['name']))
                    return
                log.error(u'An error occured while updating dataset: {}'.format(dataset))
